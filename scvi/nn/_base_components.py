    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gmm_mean = nn.Parameter(4*torch.rand(n_clusters,n_output)-2)
        self.gmm_var = nn.Parameter(2*torch.rand(n_clusters,n_output)-1)
    ...
    def forward(self, x: torch.Tensor, *cat_list: int):
        ...
        pi = self.pi_encoder(q)
        pi = self.pi_trans(pi)
        #max_index = torch.argmax(pi,axis=1)
        #result_tensor = torch.zeros_like(pi)
        ##for i in range(len(max_index)):
        #    result_tensor[i,max_index[i]] = 1
        pz_m = torch.matmul(pi,self.gmm_mean)
        pz_v = self.var_activation(torch.matmul(pi,self.gmm_var)) + self.var_eps
        # pz_m_bc = pz_m*torch.ones_like(q_m)
        # pz_v_bc = pz_v*torch.ones_like(q_v)
        pz = Normal(pz_m, pz_v.sqrt())
    
        if self.return_dist:
            return dist, latent, pz
        # return q_m, q_v, latent
